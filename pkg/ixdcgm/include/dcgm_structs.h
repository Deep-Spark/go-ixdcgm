...
extern "C"
{
    ...
    typedef enum dcgmReturn_enum
    {
        ...
        DCGM_ST_GROUP_INCOMPATIBLE   = -23,  //!< The GPUs of the provided group are not compatible with each other for
        ...
        DCGM_ST_REQUIRES_ROOT =
            -29,  //!< This operation cannot be performed when the host engine is running as non-root
...
#define dcgmConnectV2Params_version1 MAKE_DCGM_VERSION(dcgmConnectV2Params_v1, 1)
    ...
    typedef struct
    {
        ...
        unsigned int persistAfterDisconnect; /*!< Whether to persist DCGM state modified by this connection once the
                                                  ...
                                                  connection are removed once a connection goes away. 1 = do not clean
                                                up after this connection. 0 = clean up after this connection */
...
#define dcgmAllFieldGroup_version dcgmAllFieldGroup_version1
    ...
    typedef struct
    {
        ...
        unsigned int memClock;  //!< Memory Clock (Memory Clock value OR DCGM_INT32_BLANK to Ignore/Use compatible
                                //!< value with smClk)
        unsigned int
            smClock;  //!< SM Clock (SM Clock value OR DCGM_INT32_BLANK to Ignore/Use compatible value with memClk)
    } dcgmClockSet_v1;
...
#define dcgmClockSet_version dcgmClockSet_version1
    ...
    typedef struct
    {
        ...
        dcgmClockSet_t
            clockSet[DCGM_MAX_CLOCKS];  //!< Valid clock sets for the device. Upto \ref count entries are filled
...
#define DCGM_FV_FLAG_LIVE_DATA 0x00000001
     ...
     * @param values               IN: Field values. These values must be copied as they will be destroyed as soon as
     * this call returns.
...
#define DCGM_MAX_PID_INFO_NUM 16
    ...
    typedef struct
    {
        ...
        long long
            endTime;  //!< Process end time in microseconds since 1970 or reported as 0 if the process is not completed
...
#define dcgmPidInfo_version dcgmPidInfo_version2
    ...
    typedef struct
    {
        ...
        dcgmProcessUtilInfo_t computePidInfo[DCGM_MAX_PID_INFO_NUM];  //!< List of compute processes that ran during the
                                                                      //!< job 0=no process

        int                   numGraphicsPids;                         //!< Count of graphicsPids entries that are valid
        dcgmProcessUtilInfo_t graphicsPidInfo[DCGM_MAX_PID_INFO_NUM];  //!< List of compute processes that ran during
                                                                       //!< the job 0=no process
...
#define dcgmDiagResponse_version dcgmDiagResponse_version9
    ...
    typedef enum dcgmGpuLevel_enum
    {
        ...
        DCGM_TOPOLOGY_CPU =
            0x10,  //!< all devices that are connected to the same CPU but possibly multiple host bridges
...
#define DCGM_AFFINITY_BITMASK_ARRAY_SIZE 8
    ...
    typedef struct
    {
        ...
        unsigned long
            cpuAffinityMask[DCGM_AFFINITY_BITMASK_ARRAY_SIZE];  //!< affinity mask for the specified GPU
...
#define dcgmDeviceTopology_version dcgmDeviceTopology_version1
    ...
    typedef struct
    {
        ...
        unsigned long
            groupCpuAffinityMask[DCGM_AFFINITY_BITMASK_ARRAY_SIZE];  //!< the CPU affinity mask for all GPUs in the
                                                                     //!< group a 1 represents affinity to the CPU in
                                                                     //!< that bit position supports up to 256 cores
...
#define DCGM_RUN_FLAGS_FAIL_EARLY 0x0010
    ...
    typedef struct
    {
        ...
        char testParms[DCGM_MAX_TEST_PARMS]
                      [DCGM_MAX_TEST_PARMS_LEN];  //!< Parameters to set for specified tests
                                                  //!< in the format:
...
#define DCGM_PROF_MAX_FIELD_IDS_PER_GROUP_V2 64
    ...
    typedef struct
    {
        ...
        unsigned short fieldIds[DCGM_PROF_MAX_FIELD_IDS_PER_GROUP_V2];  //!< DCGM Field IDs that are part of this
