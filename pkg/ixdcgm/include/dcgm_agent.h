...
extern "C"
{
     ...
     * @note This function has a versioned argument that can be actually called with two different types. The behavior
     * will depend on the params->version value.
     ...
     *         - \ref DCGM_ST_INIT_ERROR           if DCGM encountered an error while initializing the remote client
     * library
     *         - \ref DCGM_ST_UNINITIALIZED        if DCGM has not been initialized with \ref dcgmInit
     */
     ...
     *                                         entityGroup. numEntities will contain the capacity needed to complete
     ...
     *  - \ref DCGM_ST_MAX_LIMIT         if number of groups on the system has reached the max limit \a
     * DCGM_MAX_NUM_GROUPS
     ...
     */
    dcgmReturn_t DCGM_PUBLIC_API dcgmGroupRemoveDevice(dcgmHandle_t pDcgmHandle,
                                                       dcgmGpuGrp_t groupId,
                                                       unsigned int gpuId);
     ...
     * @param fieldGroupName     IN: Unique name for this group of fields. This must not be the same as any existing
     * field groups.
    ...
    * @param pDcgmHandle            IN: DCGM Handle
    * @param groupId                IN: Group ID representing collection of one or more GPUs. Look at \ref
    dcgmGroupCreate
    *                                   for details on creating the group.
    * @param pDeviceConfig          IN: Pointer to memory to hold desired configuration to be applied for all the GPU in
    the
    ...
    * @param deviceConfigList      OUT: Pointer to memory to hold requested configuration corresponding to all the GPUs
    in
    *                                   the group (\a groupId). The size of the memory must be greater than or equal to
    hold
    ...
    *        - \ref DCGM_ST_BADPARAM             if any of \a groupId, \a type, \a count, or \a deviceConfigList is
    invalid.
    ...
    */
     ...
     * @param pDcgmHandle            IN: DCGM Handle
     * @param groupId                IN: Group ID representing collection of one or more GPUs. Look at \ref
     * dcgmGroupCreate for details on creating the group. Alternatively, pass in the group id as \a DCGM_GROUP_ALL_GPUS
     ...
     *
     * @param pDcgmHandle         IN: DCGM Handle
     * @param groupId             IN: Group ID representing collection of one or more entities. Look at \ref
     * dcgmGroupCreate for details on creating the group. Alternatively, pass in the group id as \a DCGM_GROUP_ALL_GPUS
     * to perform operation on all the GPUs or \a DCGM_GROUP_ALL_NVSWITCHES to to perform the operation on all
     * NvSwitches.
     ...
     * @param groupId             IN: Group ID representing collection of one or more entities. Look at \ref
     * dcgmGroupCreate for details on creating the group. Alternatively, pass in the group id as \a DCGM_GROUP_ALL_GPUS
     * to perform operation on all the GPUs or \a DCGM_GROUP_ALL_NVSWITCHES to perform the operation on all NvSwitches.
     * @param fieldGroupId        IN: Fields to return data for
     * @param sinceTimestamp      IN: Timestamp to request values since in usec since 1970. This will be returned in
     *                                nextSinceTimestamp for subsequent calls 0 = request all data
     * @param nextSinceTimestamp OUT: Timestamp to use for sinceTimestamp on next call to this function
     ...
     * @param fields        IN: Field IDs to return data for. See the definitions in dcgm_fields.h that start with
     * DCGM_FI_.
     ...
     * @param flags         IN: Optional flags that affect how this request is processed. Pass \ref
     * DCGM_FV_FLAG_LIVE_DATA here to retrieve a live driver value rather than a cached value. See that flag's
     ...
     *        - \ref DCGM_ST_OK                   if the call was successful
     *        - \ref DCGM_ST_BADPARAM             if a parameter is invalid
     *        - \ref DCGM_ST_REQUIRES_ROOT        if the host engine is being run as non-root, and accounting mode could
     * not be enabled (requires root). Run "nvidia-smi -am 1" as root on the node before starting DCGM to fix this.
     *
     */
     ...
     * @param pidInfo IN/OUT: Structure to return information about pid in. pidInfo->pid must be set to the pid in
     * question. pidInfo->version should be set to dcgmPidInfo_version.
     *
     * @return
     *       - \ref DCGM_ST_OK                  if the call was successful
     ...
     * @param groupId            IN: Group ID representing collection of one or more entities. Look at \ref
     * dcgmGroupCreate for details on creating the group. Alternatively, pass in the group id as \a DCGM_GROUP_ALL_GPUS
     * to perform operation on all the GPUs or \a DCGM_GROUP_ALL_NVSWITCHES to perform operation on all the NvSwitches.
     * @param systems            IN: An enum representing systems that should be enabled for health checks logically
     * OR'd together. Refer to \ref dcgmHealthSystems_t for details.
     *
     * @return
     *       - \ref DCGM_ST_OK                  if the call was successful
     *       - \ref DCGM_ST_BADPARAM            if a parameter is invalid
     *
     */
    dcgmReturn_t DCGM_PUBLIC_API dcgmHealthSet(dcgmHandle_t        pDcgmHandle,
                                               dcgmGpuGrp_t        groupId,
     ...
     * @param statusHandle   IN/OUT: Resulting status for the operation.  Pass it as NULL if the detailed error
     * information is not needed. Refer to \ref dcgmStatusCreate for details on creating a status handle.
     *
     * @return
     *        - \ref DCGM_ST_OK                   if the call was successful
     ...
     *                               group.
     * @param policy             OUT: A reference to \ref dcgmPolicy_t that will used as storage for the current
     * policies applied to each GPU in the group.
     * @param statusHandle    IN/OUT: Resulting status for the operation. Pass it as NULL if the detailed error
     * information for the operation is not needed. Refer to \ref dcgmStatusCreate for details on creating a status
     * handle.
     *
     * @return
     *        - \ref DCGM_ST_OK                   if the call was successful
     ...
     *                               This function will be called prior to any actions specified by the policy are
     * taken.
     ...
     *        - \ref DCGM_ST_VER_MISMATCH         if pDcgmDeviceTopology->version was not set to
     ...
     *                               already in use, they shouldn't be included in the bitmask. 0 means that all of the
     * GPUs in the system should be considered.
     * @param numGpus            IN: the number of GPUs that are desired from inputGpuIds. If this number is greater
     * than the number of healthy GPUs in inputGpuIds, then less than numGpus gpus will be specified in outputGpuIds.
     ...
     * @param hintFlags          IN: a bitmask of DCGM_TOPO_HINT_F_ #defines of hints that should be taken into account
     * when assigning outputGpuIds.
     *
     * @return
     *        - \ref DCGM_ST_OK                   if the call was successful
     *
     ...
     *
     */
    dcgmReturn_t DCGM_PUBLIC_API dcgmModuleGetStatuses(dcgmHandle_t             pDcgmHandle,
     ...
     *                               metricGroups->version should be set to dcgmProfGetMetricGroups_version upon
     * calling.
     *
     * @return
     *        - \ref DCGM_ST_OK                     if the request succeeds.
     ...
     *
     * @param pDcgmHandle        IN: DCGM Handle
     * @param watchFields        IN: Details of which metric groups to watch for which GPUs. See \ref
     * dcgmProfWatchFields_v1 for details of what should be put in each struct member. watchFields->version should be
     *                               set to dcgmProfWatchFields_version upon calling.
     *
     * @return
     *        - \ref DCGM_ST_OK                     if the call was successful
     *        - \ref DCGM_ST_BADPARAM               if a parameter is invalid
     *        - \ref DCGM_ST_NOT_SUPPORTED          if profiling metric group metricGroupTag is not supported for the
     * given GPU group.
     ...
     *
     */
    dcgmReturn_t DCGM_PUBLIC_API dcgmProfUnwatchFields(dcgmHandle_t             pDcgmHandle,
